...
public sealed class GroundedKnowledgeSynthesizer : IKnowledgeSynthesizer
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var summary = $"Code-observed backend behavior for the {action} action in {candidate.Area}.";
    ...
    private static IReadOnlyList<string> BuildPermissions(EvidenceFact? endpoint)
    {
        if (endpoint is null)
        {
            return [];
